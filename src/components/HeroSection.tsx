      ...
      <div className="relative">
        <div className="stagger-child mb-8 flex flex-wrap items-center gap-4 sm:mb-12 sm:gap-6">
          <div className="flex items-center gap-2 rounded-full border px-3 py-1" style={{ borderColor: "var(--border)", background: "var(--glass)" }}>
            ...
            <span className="font-mono text-[9px] font-bold uppercase tracking-wider" style={{ color: "var(--heading)" }}>
              {systemActiveLabel}
            </span>
          </div>
          <div className="hidden h-px flex-1 sm:block" style={{ background: "var(--border)" }} />
          <div className="rounded-full border px-3 py-1 font-mono text-[9px] tracking-[0.24em]" style={{ borderColor: "rgba(255,255,255,0.08)", color: "var(--accent-bright)", background: "rgba(255,255,255,0.03)" }}>
            {motionLabel}
          </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)] lg:items-end">
          <div className="max-w-4xl">
            ...
            <div className="stagger-child mt-8 flex flex-wrap items-center gap-3 sm:mt-10">
              <span className="rounded-full border px-3 py-1 font-mono text-[9px] tracking-[0.24em]" style={{ borderColor: "rgba(255,255,255,0.08)", color: "var(--accent-bright)", background: "rgba(255,255,255,0.03)" }}>
                {sectorLabel}
              </span>
              <span className="rounded-full border px-3 py-1 font-mono text-[9px] tracking-[0.24em]" style={{ borderColor: "rgba(255,255,255,0.08)", color: "var(--cyan)", background: "rgba(255,255,255,0.025)" }}>
          ...
          <div className="stagger-child relative overflow-hidden rounded-[28px] border p-5 sm:p-6 hero-signal-panel" style={{ borderColor: "rgba(255,255,255,0.08)", background: "linear-gradient(180deg, rgba(255,255,255,0.04), rgba(255,255,255,0.015))" }}>
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between gap-4">
                <p className="font-mono text-[10px] tracking-[0.28em]" style={{ color: "var(--accent-bright)" }}>
                  {locale === "zh" ? "[ 首页信号面板 ]" : "[ HERO_SIGNAL_PANEL ]"}
                </p>
                <span className="rounded-full border px-2.5 py-1 font-mono text-[8px] tracking-[0.24em]" style={{ borderColor: "rgba(255,255,255,0.08)", color: "var(--cyan)" }}>
                  LIVE FEED
                </span>
              </div>
              ...
              <div className="mt-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {heroStats.map((m) => (
                    <div key={m.label} className="hero-signal-card rounded-2xl border p-4 sm:p-5" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)" }}>
                      <p className="font-mono text-[7px] sm:text-[8px] tracking-[0.2em]" style={{ color: "var(--muted)" }}>
                        {m.label}
                      </p>
                      <p className="mt-2 font-mono text-sm sm:text-[18px]" style={{ color: "var(--heading)" }}>
                        {m.value}
                      </p>
                    </div>
                  ))}
                ...
                <div className="rounded-2xl border px-4 py-3 font-mono text-[9px] tracking-[0.18em] overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.025)", color: "var(--muted)" }}>
        ...
        <div className="grid gap-px sm:grid-cols-3" style={{ background: "var(--border)" }}>
          {[
            ...
            <div key={item.label} className="px-4 py-4 sm:px-5" style={{ background: "var(--background-elevated)" }}>
              <p className="font-mono text-[7px] tracking-[0.22em]" style={{ color: "var(--muted)" }}>
                {item.label}
              </p>
              <p className="mt-2 text-[13px] leading-6" style={{ color: "var(--body)" }}>
                {item.value}
              </p>
            </div>
          ))}
