...
export function CommandCenterPanel({
  ...
  const title = locale === "zh" ? "[ 调度中心概览 ]" : "[ DISPATCH_CENTER_OVERVIEW ]";
    ...
    >
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="stagger-child flex items-center gap-3">
          ...
          <p className="font-mono text-[13px] font-bold tracking-[0.3em]" style={{ color: "var(--accent-bright)" }}>
            {title}
          </p>
        </div>
        <div className="stagger-child flex flex-wrap items-center gap-2">
          <span className="rounded-full border px-2.5 py-1 font-mono text-[11px] tracking-[0.24em]" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", color: "var(--cyan)" }}>
            {pulseLabel}
          </span>
          <span className="rounded-full border px-2.5 py-1 font-mono text-[11px] tracking-[0.24em]" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", color: "var(--accent-bright)" }}>
            {layerLabel}
          </span>
        </div>
      </div>
        ...
        <div className="stagger-child relative overflow-hidden p-5 sm:p-6 lg:p-7" style={{ background: "var(--background-elevated)" }}>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between gap-4">
              <p className="font-mono text-[13px] font-bold tracking-[0.28em]" style={{ color: "var(--muted)" }}>
                {locale === "zh" ? "信号汇总" : "SIGNAL_AGGREGATION"}
              </p>
              <span className="rounded-full border px-2.5 py-1 font-mono text-[11px] tracking-[0.22em] command-badge" style={{ borderColor: "rgba(255,255,255,0.08)", color: "var(--emerald)" }}>
              ...
              {signals.map((signal) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-mono text-[11px] tracking-[0.22em]" style={{ color: "var(--muted)" }}>
                      {signal.label[locale]}
                    </p>
                  ...
                  <p className="mt-3 font-mono text-[26px] sm:text-[30px]" style={{ color: toneColor(signal.tone) }} suppressHydrationWarning>
                   {signal.value}
                 </p>
                </div>
              ))}
              ...
              <div className="rounded-[22px] border px-4 py-4 command-card" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.025)" }}>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {["L1", "L2", "L3", "L4", "L5", "L6"].map((cell, index) => (
                    <div
                      key={cell}
                      className="rounded-xl border px-2 py-2 text-center font-mono text-[13px] command-matrix-cell"
                      style={{
        ...
        <div className="stagger-child relative overflow-hidden p-5 sm:p-6 lg:p-7" style={{ background: "var(--background-elevated)" }}>
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="rounded-full border px-2.5 py-1 font-mono text-[11px] tracking-[0.22em] command-badge" style={{ borderColor: "rgba(255,255,255,0.08)", color: "var(--accent-bright)" }}>
                {locale === "zh" ? "实时运行" : "LIVE OPS"}
              </span>
            </div>
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-[12px] tracking-[0.18em]" style={{ color: "var(--heading)" }}>
                        {event.code}
                      </span>
                    </div>
                    <span className="font-mono text-[11px]" style={{ color: "var(--muted)" }} suppressHydrationWarning>
                      {event.timestamp}
                    </span>
                  </div>
